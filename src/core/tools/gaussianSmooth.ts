...
function convolve1D(
  ...
  for (let idx = 0; idx < totalSize; idx++) {
    ...
    for (let k = 0; k < kernelSize; k++) {
      ...
      const sampleIdx = sampleCoords[0] + sampleCoords[1] * strideY + sampleCoords[2] * strideZ;
...
export function validateGaussianSmoothParams(params: GaussianSmoothParams): string | null {
  if (params.sigma <= 0) {
