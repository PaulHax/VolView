...
// Chunk 22: the live-only "new job result" badge set. Born-persistent review
// model (decisions.md D6) — the badge is the ONLY review surface; the group
...
describe('jobResultReview store', () => {
  ...
  it('is live-only: exposes no serialize/deserialize (never enters the zip)', () => {
    const store = useJobResultReviewStore();
    expect(
      (store as unknown as { serialize?: unknown }).serialize
    ).toBeUndefined();
    expect(
      (store as unknown as { deserialize?: unknown }).deserialize
    ).toBeUndefined();
  });
});

// The "reject" acceptance (Chunk 22 work item 3): a job result is a NORMAL group
...
describe('deleting an auto-shown job result (existing delete UI)', () => {
  ...
  it('removeGroup deletes a job (source-tagged) group and the badge is cleared', () => {
    ...
    // Reject = the existing delete primitive + badge dismiss (deleteGroup).
    review.dismiss(id);
