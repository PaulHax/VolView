...
export interface ProcessAlgorithm {
  compute: (segImage: vtkLabelMap, activeSegment: number) => Promise<TypedArray>;
}
...
export const useProcessStore = defineStore('process', () => {
  ...
  async function computeProcess(
    ...
    try {
      const outputScalars = await algorithm.compute(segImage, paintStore.activeSegment);
  ...
  function togglePreview() {
    ...
    if (state.step === 'previewing') {
      ...
      const scalarsToShow = newShowingOriginal ? state.originalScalars : state.processedScalars;
