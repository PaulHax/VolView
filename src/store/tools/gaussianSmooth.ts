...
} from '@/src/core/tools/paint/gaussianSmooth.worker';
...
async function getWorker() {
  if (!workerInstance) {
    ...
    const worker = new Worker(
      new URL('@/src/core/tools/paint/gaussianSmooth.worker.ts', import.meta.url),
      { type: 'module' }
    );
...
export const useGaussianSmoothStore = defineStore('gaussianSmooth', () => {
  ...
  async function computeAlgorithm(segImage: vtkLabelMap, activeSegment: number): Promise<TypedArray> {
    const params = {
      sigma: sigma.value,
      label: activeSegment,
    };
