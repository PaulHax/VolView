// ---------------------------------------------------------------------------
// Crop-tool → `bounds` parameter binding (contract Seam 2 — the imaging-native
// `bounds` field; WORKORDER WI1 "bounds binds from the crop tool").
//
...
import type { Bounds } from '@/processing-contract';
...
// Map each LPS axis to its index-space column (the crop store keys its ranges
// by LPS axis; the columns place them into an index-space point).
export const cropPlanesToWorldBounds = (
  ...
  const columns = {
    Sagittal: lpsOrientation.Sagittal,
    Coronal: lpsOrientation.Coronal,
    Axial: lpsOrientation.Axial,
  };
  ...
  planes.Sagittal.forEach((s) => {
    planes.Coronal.forEach((c) => {
      planes.Axial.forEach((a) => {
        ...
        index[columns.Sagittal] = s;
        index[columns.Coronal] = c;
        index[columns.Axial] = a;
