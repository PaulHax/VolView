// ---------------------------------------------------------------------------
// Spec-driven form model (contract Seam 2 — task description; WORKORDER WI1/WI2).
//
...
// are minted from provenance in Chunk 8; `bounds` values are bound from the
// crop tool by the caller. This module stays pure (no store, no Vue).
//
// House rules: functional style; `type`, not `interface`.
// ---------------------------------------------------------------------------

import { taskParameterSchema } from '@/processing-contract';
...
// (its value is minted from provenance in Chunk 8); `bounds` is left unbound
  ...
  model.fields.forEach((f) => {
    switch (f.kind) {
      ...
      case 'bounds':
        ...
        break;
      case 'sourceRef':
        // Value is minted from provenance in Chunk 8; rendered unbound here.
        values[f.id] = null;
        break;
