// ---------------------------------------------------------------------------
// Seam-1 labelmap input binding (contract "Seam 1 — inputs", CLIENT half;
// WORKORDER Chunk 15; decision D10).
//
// The one input the client authors itself. A `labelmap` sourceRef param
// (`accepts: ["labelmap"]`, authored by the facade's Slicer-XML→spec
...
// facade-minted `{uris}`), so this module does NOT mint from provenance. It
...
//
// Fallback chain (WI1), deterministic, no picker:
...
// binds purely by the open type tag off the zod-validated spec (D2/Seam 2) —
...
export const bindLabelmapInputs = (
  ...
  // Fail closed: no bindable segment group for the bound background.
...
// ---------------------------------------------------------------------------
// Mint the Seam-1 labelmap value from the staging response
// ---------------------------------------------------------------------------

// The bound labelmap value is `{ type: "labelmap", uris }` — the uris come from
// the facade STAGING RESPONSE (facade-minted), never from DataSource
// provenance. `format` is intentionally omitted (advisory only, D10): the
