...
const rowNamed = async (root: string, name: string, timeout?: number) => {
 ...
 */
export const waitForSegmentContent = async (
  name: string,
  timeout = DOWNLOAD_TIMEOUT
) => {
  const row = await rowNamed(SEGMENT_LIST, name, timeout);
  await browser.waitUntil(
    async () => (await $(SEGMENT_LIST).getAttribute('aria-busy')) !== 'true',
    { timeout, timeoutMsg: 'Expected the scene to finish loading' }
  );
