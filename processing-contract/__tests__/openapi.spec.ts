...
// The published OpenAPI is the facade's obligation surface (WORKORDER Chunk 23).
...
// facade obligation without reading girder_volview source).
...
describe('single source — published shapes track the source of truth', () => {
  it('NeutralJobStatus.state enum equals the runtime JOB_STATES', () => {
    ...
    const state = get(get(status, 'properties'), 'state');
    expect(get(state, 'enum')).toEqual([...JOB_STATES]);
  });
