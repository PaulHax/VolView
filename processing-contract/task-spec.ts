// ---------------------------------------------------------------------------
// VolView task-spec schema (contract Seam 2 — task description; decision D2).
//
// This is VolView's OWN, zod-validated task description. A backend never speaks
// this shape natively: a server-side facade TRANSLATES its native task format
...
// and the facade (a JSON-Schema generated from this source) validate against.
//
...
// consumer (our renderer). See client-processing-contract.md, "Seam 2".
//
// House rules: functional style; `type`, not `interface`. Additive-only —
// D6 fields (guidance / interactive semantics) and the future binding
// descriptor must extend this schema AGAINST `specVersion`, never mutate it.
// ---------------------------------------------------------------------------
...
// Seam-1 semantic type tag. OPEN vocabulary (no closed server enum, D10): the
...
// unknown kind fails validation (Chunk 5 negative fixture). The engine
// (Chunk 7) reuses the per-kind pieces to hide an unknown param and refuse
...
// Imaging-native field kind: the Seam-1 input. `accepts` is a list of the open
// type tags this input binds (absent/`scalar` `<image>` → `["image"]`,
// `<image type="label">` → `["labelmap"]`; the facade authors the mapping).
...
// rigor over the structural JSON-Schema view generated for the facade.
...
// (e.g. `.nii.gz`). The facade authors these; the client maps them to result
// intents (Chunk 11).
